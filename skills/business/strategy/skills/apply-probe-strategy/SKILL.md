...
tags: [business, strategy, thirty-six-stratagems, probe, testing, pre-commitment, risk-reduction, beat-grass-snake]
verified: true
---

# Apply Probe Strategy
...
### Step 1: Identify the specific uncertainty the probe must resolve
...
### Step 2: Design the probe to provoke genuine response

A probe that is too hypothetical produces hypothetical responses. The probe must be real enough that:
- A customer who sees a smoke test ad responds because they actually want to buy, not because they're being polite
- A competitor who sees a market announcement responds with a real counter-move, not a PR statement
- A regulator who receives a pre-consultation briefing responds with substantive feedback, not a form letter

Indicators of a probe that is too hypothetical:
- Customer research question: "Would you buy this if it existed?" (always yes; no purchasing commitment)
- Competitor probe: "What do you think our competitor would do?" (speculation; not observed behaviour)
- Regulatory probe: Asking external lawyers rather than the regulator directly

### Step 3: Execute the probe and observe — including what does NOT react

The probe's output has two parts:
1. **What reacted:** The competitor's counter-announcement, the customer segment's pre-sign-up rate, the regulator's letter
2. **What did not react:** Competitors who did not respond may be unaware, dismissive, or preparing a delayed response — all meaningful signals
...
### Step 4: Interpret what the probe reveals before committing

Before moving from probe to full commitment, translate the probe's output into a decision-relevant insight:

- **Probe confirmed the assumption:** Proceed with the main move, adjusted for anything the probe revealed about execution details
- **Probe revealed partial risk:** Redesign the move to address the specific risk the probe revealed; do not proceed with the original plan unchanged
- **Probe revealed a disqualifying risk:** Abandon or substantially rethink the move; the cost of proceeding now is greater than the cost of redesigning
- **Probe was inconclusive:** The probe did not reveal the needed information; redesign the probe and run again before committing

The probe result must change something about the main move — otherwise the probe was not actually decision-relevant.

### Step 5: Move to full commitment only after the probe resolves the uncertainty
...
- Commit fully if the probe was positive
- Adjust and commit if the probe revealed manageable risks
- Do not run additional probes on uncertainties the first probe already resolved — that is delay, not discipline

## Rules

- One critical uncertainty per probe. Probes designed to answer too many questions produce unclear results. Isolate the highest-stakes uncertainty; design the probe for that question; run it; then address secondary uncertainties.
- The probe must be real enough to provoke genuine response. A probe that cannot provoke a real response cannot reveal real information. If you cannot design a real enough probe, the probe approach is not the right tool — consider scenario planning or expert interviews instead.
- Absence of response is signal. Do not interpret no response as confirmation. Analyse why the snake did not move: unaware, dismissive, or preparing.
- Act on the probe result. Running a probe and ignoring its output because it is inconvenient is worse than not probing. The probe's value is in the decision it enables, not the data it generates.
- Probing is not permission to delay indefinitely. Some decisions cannot wait for probe results. Know in advance the maximum time you will spend on the probe phase before committing regardless.
