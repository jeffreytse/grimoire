...
### Step 1: Identify the expert judgment that drives superior outcomes
...
- Identify the top quartile of performers in the domain (sales reps, engineers, support agents, investment analysts)
- Measure the performance gap between the top quartile and the median: is it large enough to justify the doctrine-building investment? If the top quartile is 20% better, doctrine has limited leverage; if it is 3× better, doctrine has enormous leverage
- Observe top performers in action across multiple situations — do not ask them to explain their approach; observe what they actually do. Top performers are often unable to articulate the rules behind their intuitive decisions (the knowledge is tacit, not explicit)
...
### Step 2: Document decision patterns as explicit rules with situational triggers
...
- **Identify the trigger:** What situation, condition, or signal activates this response?
- **Specify the response:** What exactly does the expert do? Not "they communicate clearly" but "they ask three specific questions in this order"
- **Identify the variations:** Under what conditions does the standard response not apply?
...
### Step 3: Specify transitions — when to shift from one rule to another
...
### Step 4: Test with average practitioners — the doctrine is not valid until they can execute it
...
- Give average practitioners the written doctrine without any additional coaching
- Observe where they succeed and where they fail
- Where they fail: is the failure because the doctrine is incomplete (missing a rule or a trigger), ambiguous (the rule is unclear), or simply requires knowledge the practitioner does not have (a training problem, not a doctrine problem)?
...
### Step 5: Build in the failure modes — explicit exceptions and escalation paths
...
For the doctrine:
- Identify the situations where the standard rules do not apply — edge cases, novel situations, situations that require judgment beyond what the doctrine encodes
- Specify explicitly what to do in those situations: either "use judgment" (with guidance on what factors to weigh) or "escalate" (with specification of to whom and with what information)
- The failure mode specification is as important as the standard doctrine — practitioners who hit an undocumented failure mode with no guidance will either freeze or improvise badly

### Step 6: Build feedback loops — doctrine must update from field results
...
- **Failure loop:** When a practitioner follows the doctrine and fails, investigate whether the failure reveals a doctrine gap (update the doctrine) or a training gap (update the training)
- **Success loop:** When a practitioner deviates from the doctrine and succeeds, investigate whether the deviation reveals a doctrine gap (update the doctrine) or a tactical innovation (add to the doctrine)
...
### Step 7: Assign ownership and protect from entropy
...
- The owner reviews doctrine quarterly against field results
- No practitioner may update the doctrine unilaterally — changes go through the owner, who validates them against performance data before incorporating them
- The owner removes rules that have stopped working, even if they were added by senior people
- Practitioners can flag potential doctrine updates; the owner evaluates them
...
- Build doctrine from observation, not from interviews. What top performers say they do and what they actually do are often different. Observe multiple performances before converting patterns to rules.
- The doctrine is not validated until average practitioners can execute it. Expert validation is necessary but not sufficient. The doctrine must work for the people who need it, not for the people who already know it.
- Every rule must have a trigger. "Be responsive to customers" is not doctrine — it is aspiration. "Within two hours of a customer message that contains the word 'urgent' or describes a production issue, respond with acknowledgment and estimated resolution time" is doctrine.
- Specify the failure modes explicitly. Doctrine without documented edge cases and escalation paths produces confident failures — practitioners who apply the wrong rule to the wrong situation because they have no guidance on when the standard rules do not apply.
- Assign ownership and review cadence. Doctrine that is not maintained becomes stale and then counterproductive. The ownership must be specific, the review must be scheduled, and the performance data must flow back to the owner.
