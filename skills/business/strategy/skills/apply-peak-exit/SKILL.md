...
Feng Menglong identified this as a form of 上智 (superior wisdom) precisely because it is so rare: the psychological pull to continue when winning is stronger than the pull to continue when losing. Stopping while winning requires overcoming the sunk-cost illusion in reverse — "I'm still making returns, why stop?"
...
3. **Identify the leading indicators of peak.** The lagging indicators (revenue decline, customer churn, team attrition) arrive after the peak. The leading indicators arrive before:
   - Structural: competitors entering the space; regulatory attention increasing; technology curve shifting
   - Market: sentiment reaching extreme optimism (everyone is bullish); new entrants flooding the space; deal multiples expanding past historical norms
   - Operational: marginal returns on additional investment declining; team quality of new hires declining; customer acquisition costs rising faster than lifetime value
   - Relational: key customers beginning to evaluate alternatives; key team members receiving outside offers
   Map the leading indicators specific to each position. Define the threshold at which you would exit.

4. **Define your exit threshold before you need it.** Decisions made under the pressure of a potentially declining position are subject to status quo bias, loss aversion, and ego investment. Define the exit trigger in advance, in writing, when you are not under pressure. "We will exit this market if: [specific measurable condition]." Pre-committing to the threshold makes it harder to rationalize away when the threshold is reached.

5. **Apply the peak test at regular intervals.** Ask: "If we were entering this position today, would we?" If the answer is no — if the same investment at current conditions would not be attractive — that is a signal you are past peak. The position's historical returns are irrelevant to the exit decision; only the forward returns matter.

6. **Identify and counter the psychological barriers to exit.** The barriers to peak exit are predictable: (a) "We're still making money" — true, but the question is whether you'll make more by continuing or by redirecting; (b) "We've invested so much" — sunk cost; (c) "What will people think?" — identity and status; (d) "It might turn around" — hope without evidence. Name the barrier explicitly before making the exit decision. If your reason for continuing is one of these four, apply more scrutiny.

7. **Execute the exit before the external signal confirms the need.** Peak exit by definition means exiting before the decline is visible in outcomes. If you wait for the lagging indicator to confirm you should have exited, you have missed the peak exit. The exit decision should feel slightly premature — because it is: the peak hasn't been confirmed yet, because peaks are only visible in retrospect.

## Rules

- "It's still working" is not a reason to continue. The question is whether the forward return justifies the opportunity cost of continued investment. A declining return on an increasing base is still "working" — but it may not be optimal.
- Pre-committed exit triggers are more reliable than in-the-moment judgment. Write them down before you need them. When the trigger condition is reached, exit.
- Social confirmation that you should exit is itself a lagging indicator. When everyone agrees it's time to sell, the exit price has already compressed. Peak exit requires acting on leading indicators before consensus forms.
- Do not confuse peak exit with premature exit. Peak exit requires evidence of leading indicators approaching peak, not mere discomfort with a winning position. The skill is identifying the peak, not avoiding success.
...
- **Waiting for the lagging indicator:** Revenue decline, customer churn, competitive loss are all lagging indicators — they confirm that peak has passed, not that you are approaching it. By the time lagging indicators signal exit, the optimal exit moment has passed.
- **Mistaking "still positive" for "still optimal":** A declining rate of return on an increasing base is still positive in absolute terms. The exit decision is not "is this still good?" but "is continued investment better than the alternative?"
- **Exiting after receiving external confirmation:** The moment when everyone agrees you should exit is the moment exit is most expensive. Peak exit precedes consensus by design.
- **Redefining the exit trigger when it is reached:** The sunk-cost and identity barriers predictably cause rationalization when pre-committed triggers are reached. "We said we'd exit at X, but actually X means something slightly different now." If your reasoning for changing the trigger involves the current investment level, relationship, or status, it is rationalization.
- **Applying peak-exit to every position:** This skill applies to positions where the winning mechanism is time-bounded — where a structural advantage will eventually be competed away, or a market condition will eventually normalize. It does not apply to positions with durable compounding advantages (network effects that strengthen with time, cost advantages that grow with scale). Diagnose whether the position is time-bounded before applying.
