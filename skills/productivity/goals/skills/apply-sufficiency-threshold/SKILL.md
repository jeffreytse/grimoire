---
name: apply-sufficiency-threshold
description: Use when deciding whether to continue optimizing, pursuing, or acquiring — to define "enough" as an explicit target before you start, and treat reaching that threshold as success rather than continuing to maximize, because maximizing without a defined endpoint produces worse outcomes than satisficing with a clear one
source: "道德经 第三十三章、第四十六章 (Laozi, ~6th–4th century BC) — 知足者富 (Ch. 33), 知足常乐 (Ch. 46); Herbert Simon satisficing theory (1956, Nobel Prize 1978); Barry Schwartz \"The Paradox of Choice\" (2004); Morgan Housel \"The Psychology of Money\" (2020) — the 'enough' number; Kurt Vonnegut / Joseph Heller 'enough' concept; Warren Buffett on the costs of not knowing when to stop"
tags: [decision-making, goals, optimization, behavioral-economics, risk-management]
verified: true
---

# Apply Sufficiency Threshold
...
1. **Define your sufficiency threshold before beginning.** Before starting any optimization, pursuit, or acquisition, answer: "At what level of outcome would I stop and say this is enough?" The definition must be:
   - **Specific:** Not "when I'm financially secure" but "when I have $X in liquid assets generating $Y annually"
   - **Pre-committed:** Written down before you start, not revised under the influence of current progress
   - **Decision-triggering:** Reaching the threshold should automatically trigger a stop decision, not a re-evaluation
   Failing to define a specific threshold before starting is the most common reason people continue past the point where continuing is beneficial.

2. **Distinguish the threshold from the target.** A target ("grow revenue to $10M") is a goal you're working toward. A sufficiency threshold is the level at which you'd stop pursuing more even if more were available. In many decisions, these are the same. In others, they're different: you may target $10M revenue but your sufficiency threshold is "profitable with enough runway to build the next product" — you wouldn't burn cash chasing $10M if $6M profitable met the threshold.

3. **Apply the threshold test to the current decision.** When deciding whether to continue, ask the threshold test: "Has the current outcome met my sufficiency threshold?" If yes: stop. Do not re-evaluate the threshold because progress looks good. The point of pre-commitment is that it prevents rationalization at the moment of decision.

4. **Identify and counter the psychological barriers to stopping.** The barriers to stopping at sufficiency are predictable:
   - **Momentum bias:** "We've been succeeding, why stop?" Success feels like evidence that continuing is right.
   - **Relative comparison:** "Others are doing more." Comparison resets the threshold upward without a principled basis.
   - **Sunk-cost continuation:** "We've invested so much." Past investment is irrelevant to the forward decision.
   - **Identity attachment:** "This is what I do." Stopping feels like abandoning identity.
   Name the barrier before deciding whether to continue past the threshold. If your reason for continuing is one of these four, stop.

5. **Apply sufficiency threshold to time, not just outcomes.** Many optimization processes run indefinitely because they measure output quality but not time invested. Define a time-based sufficiency threshold too: "I'll spend X hours on this decision; when X hours are spent, I'll accept the best option found so far." Simon's research shows that time-bounded satisficing (stop after X search effort) produces better real-world outcomes than open-ended maximizing.

6. **Revisit thresholds only on principle, not on current progress.** A threshold set when you started should not be revised because you are making progress and further progress seems available. Revising upward because progress is good is the failure mode this skill prevents. Revise thresholds only when the underlying conditions that produced the original threshold have changed significantly (the goal context has shifted, not just the current outcome level).

## Rules

- Define the threshold before you start, not when you approach it. Thresholds defined while approaching the current level are contaminated by current state and availability bias. The pre-commitment must be genuinely prior.
- "More is available" is not a reason to take more. The sufficiency threshold applies specifically when more is available. If nothing more were available, no discipline would be required. The threshold activates in the presence of available excess.
- Satisficing is not underperformance. Simon's research established that satisficing is the mathematically correct strategy in decisions with incomplete information — not a concession to laziness. "I stopped at good enough" is a rigorous decision, not a failure to optimize.
- The threshold is about stopping, not about limiting ambition. You can have ambitious targets and a defined sufficiency threshold. The ambitious target drives the work; the sufficiency threshold determines when the work is done.
...
- **Setting the threshold after reaching current progress:** "I'll stop at 20% above where I am now." This is not a sufficiency threshold — it is an anchoring bias. Thresholds must be set in advance relative to the underlying need, not relative to current position.
- **Conflating the threshold with the minimum:** "Enough is the minimum I can survive with." Sufficiency is not the floor — it is the level at which continuing to pursue more costs more than it returns. This is almost always higher than the survival minimum.
- **Revising the threshold upward without principled justification:** The most common failure mode. "We said we'd stop at X, but now X seems low given the opportunity." If the original rationale for X was sound and the underlying conditions haven't changed, revision is rationalization, not recalibration.
- **Applying the threshold to outcomes but not to effort:** Defining "enough" for output without defining "enough" for input (time, money, attention) leaves the optimization open-ended on the resource side. Define thresholds for both: what outcome level is sufficient, and how much resource input is sufficient to get there.
