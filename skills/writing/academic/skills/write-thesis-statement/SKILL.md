---
name: write-thesis-statement
description: Use when drafting or refining the central argument for an academic essay, research paper, or analytical piece
source: "Purdue OWL Academic Writing Guidelines; They Say / I Say (Graff & Birkenstein, 2006); Harvard Writing Center thesis guidance"
tags: [academic-writing, thesis, argument, essay, research, rhetoric]
---

# Write Thesis Statement

Craft a single defensible claim that defines the paper's scope, stakes, and argument — and makes a reader want to see it proven.

## Why This Is Best Practice

**Adopted by:** Purdue OWL (standard reference for over 30 million academic writers); Harvard College Writing Program; MLA and APA style guides as foundational to academic argument

**Impact:** Writing programs universally identify a weak or missing thesis as the leading cause of unfocused papers; Harvard Writing Center reports thesis revision as the intervention that most improves paper quality

A thesis is not an observation or a topic — it is a claim someone could disagree with. Without a falsifiable thesis, a paper has no argument, only summary. The thesis governs every paragraph that follows: if a paragraph does not advance the thesis, it does not belong in the paper.

## Steps

1. **Identify your topic and angle** — start broad (the topic) then locate the specific claim you will make about it (the angle); the angle is where the thesis lives
2. **Make it arguable** — if no one could reasonably disagree, it is a fact, not a thesis; add a "because" or "although" clause to generate tension
3. **State the stakes** — a strong thesis answers "so what?"; explain why the argument matters to the field, to policy, or to human understanding
4. **Specify the scope** — the thesis should signal what the paper will and will not cover; overscoped claims invite shallow analysis
5. **Apply the "although/because" test** — rewrite as: "Although [counterargument], [claim] because [reason]"; if the sentence collapses, the thesis lacks argumentative structure
6. **Position it correctly** — in most academic formats, the thesis belongs at the end of the introduction, after context is established

## Rules

- The thesis must make a claim, not announce a topic ("This paper examines X" is not a thesis)
- Every body paragraph must begin with a topic sentence that directly advances the thesis
- Do not hedge the thesis into meaninglessness: "Some people might argue that perhaps…" signals lack of conviction
- The thesis must be provable with evidence available to the writer

## Examples

**Weak:** "Social media has effects on teenagers."
**Strong:** "Although social media platforms create documented mental health risks for adolescent users, prohibiting access without parallel investment in digital literacy education will increase, not decrease, harm — because prohibition without skills leaves teenagers less equipped to navigate online environments they will encounter regardless."

## Common Mistakes

- Stating a fact instead of an argument: facts require no proof, so the paper has nothing to do
- Thesis too broad to support in the assigned length: a 10-page paper cannot prove a claim that requires a book
- Burying the thesis in the middle of the introduction: readers lose orientation and cannot evaluate whether evidence is relevant
