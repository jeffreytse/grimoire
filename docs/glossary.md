...
One of the two required testing passes for a skill. Runs `review-skill` against the SKILL.md to verify all required format criteria are met. The other pass is *behavioral testing*.
...
**Contributor**
Anyone opening a PR to add or revise a skill. Contributors write skills and self-review with `review-skill` but cannot merge their own PRs. See also: *maintainer*.
...
**Deprecated skill**
A skill marked for removal because the practice has been superseded or never qualified. Deprecation is a two-step process: (1) add deprecation notice and `deprecated: true` in marketplace.json, (2) remove after one release cycle. Use `deprecate-skill` to guide the process.
...
**Four axes**
The four required dimensions that a skill's tags must cover: *problem keyword*, *tool/method*, *role/context*, and *outcome*. Tags missing any axis weaken `suggest-practice` routing. See [tags.md](./tags.md) for accepted values per axis.
...
**NEEDS-REVISION**
One of three `review-skill` verdicts. The skill has specific fixable findings but doesn't require a full rewrite. Each finding identifies the criterion, what's wrong, and how to fix it. Use `revise-skill` to address NEEDS-REVISION findings. See also: *PASS*, *REJECT*.
...
**PASS**
One of three `review-skill` verdicts. All criteria met — the skill is ready to open a PR. See also: *NEEDS-REVISION*, *REJECT*.
...
**REJECT**
One of three `review-skill` verdicts. A required field is missing or a core criterion fails — the skill cannot be routed correctly by `suggest-practice` and fails the basic contract of the skill format. Fix the blocking issue before addressing anything else. REJECT on frontmatter blocks further evaluation. See also: *PASS*, *NEEDS-REVISION*.
...
**Seed skill**
A skill included when a new domain is first added to grimoire. Minimum 2 seed skills required for any new domain PR. Seed skills demonstrate the domain's scope and prove the domain structure works before merge. All seed skills must pass `review-skill` before the domain PR opens.

**Skill**
A SKILL.md file encoding a single best practice. A skill covers one concept, has a specific audience, names specific adopters, cites measurable impact, and passes `review-skill`. Not a prompt, not a template, not a guideline — an evidence-backed instruction.
...
The situation that causes a skill to activate. Defined in the skill's `description` frontmatter field. Must describe WHEN to use the skill, not WHAT the skill does. `suggest-practice` uses triggering conditions to route situations to the right skill.
...
**Verdict**
The output of `review-skill`: PASS, NEEDS-REVISION, or REJECT, plus a per-criterion findings table. Verdicts are deterministic — the same skill file produces the same verdict regardless of who runs the review.
