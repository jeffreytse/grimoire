---
name: design-differentiated-instruction-plan
...
# Design Differentiated Instruction Plan
...
2. **Identify the essential learning outcomes** — Write 1–3 non-negotiable learning objectives that all students must reach, regardless of differentiation pathway. These are the "big ideas" (Tomlinson's term) that anchor the unit. Differentiation adjusts the route to these outcomes, not the outcomes themselves. Use Bloom's Taxonomy to specify the cognitive level required (remember, understand, apply, analyze, evaluate, create).

3. **Differentiate content (what students learn)** — Vary the complexity and abstractness of input materials by tier. Approaching: concrete, visual, simplified-vocabulary texts, graphic organizers pre-filled with scaffolds. At grade level: standard texts and resources. Above grade level: primary sources, extended readings, abstract conceptual challenges. Use the same core concepts across all tiers — just change the scaffolding and complexity of examples.

4. **Differentiate process (how students make sense of content)** — Design flexible activities that match cognitive demand to readiness. Use tiered assignments (same skill, varying complexity), learning menus (student-choice within teacher-defined options), and learning contracts for advanced students. Apply UDL's principle of multiple means of engagement: offer movement, collaborative, independent, and inquiry-based process options within the same lesson.

5. **Differentiate product (how students demonstrate understanding)** — Allow multiple ways for students to show mastery: written report, oral presentation, visual poster, screencast, model, or performance. Define quality criteria with a clear rubric so that different product formats are assessed against the same learning standards. Product differentiation motivates students by honoring diverse strengths.

6. **Design flexible grouping structures** — Do not use fixed ability groups. Instead, use fluid grouping that changes by task: readiness groups (tiered tasks), interest groups (project choice), random groups (building community), and individual work (independent practice). Plan group transitions in advance so time is not lost in transitions. Heterogeneous grouping for collaborative problem-solving, homogeneous grouping for targeted skill instruction.

7. **Prepare tiered materials in advance** — Build the three content tiers, the flexible product options, and the scaffolding tools (graphic organizers, sentence starters, anchor charts, glossaries) before the lesson. Color-code materials by tier for efficient distribution (do not label tiers by ability names — this stigmatizes). Prepare extension tasks for early finishers so advanced learners are never idle.

8. **Build in ongoing formative assessment** — Design check-in points every 10–15 minutes of instruction: thumbs up/down, whiteboard responses, exit tickets, or quick partner discussions. Use results to dynamically adjust grouping and pacing within the lesson. A student who masters the content mid-lesson should move to extension tasks, not sit idle.

9. **Manage the differentiated classroom physically** — Organize the room for flexible transitions: tables that can be reconfigured for group sizes from 2 to 6, designated quiet zones, and easily accessible materials stations. Post the day's activity options visually so students can self-navigate. Train students in procedures for getting materials, transitioning between tasks, and seeking help without interrupting other groups — this management infrastructure makes differentiation sustainable.

10. **Evaluate and iterate on the differentiation design** — After the lesson, review which students from each tier met the learning objective (from exit tickets or assessments). Adjust tier placement for the next lesson based on new data. Identify which product/process options produced the strongest demonstrations of understanding and build those into future designs. Document what worked in lesson notes for future iterations.

## Rules

- Never use fixed ability groups long-term — research consistently shows fixed grouping entrenches achievement gaps; regroup students at every unit or more frequently based on new assessment data
- Differentiation adjusts the pathway, not the destination — all students must be held to the same essential learning standards; reducing the rigor of objectives for struggling learners is not differentiation, it is lowering expectations
- Pre-assessment is non-negotiable before a differentiated unit — designing tiers without knowing where students are is guesswork that will misplace students
- Materials for all tiers must be intellectually respectful — "lower" tier materials must engage the same big ideas, not simpler, trivial tasks; the difference is scaffolding, not significance
- Flexible grouping must be the norm; students who are always in the same group will perceive it as tracking

## Common Mistakes

- **Differentiating only for struggling learners** — Many teachers plan extensions only after the lesson and for only a few students; advanced learners deserve as much intentional differentiation as those who are struggling, built into the lesson design from the start.
- **Using differentiation as permanent sorting** — Treating readiness tiers as fixed categories ("the low group") becomes de facto tracking; tier assignments must change as student performance changes, sometimes within a single unit.
- **Creating too many simultaneous tracks** — Designing six different versions of every activity is unsustainable; start with two or three tiers and use menus and choice boards to create apparent variety within that manageable structure.
- **Neglecting management systems** — Launching differentiated activities without trained procedures for transitions and materials leads to chaos; spend the first weeks of the year building the routines that make differentiated learning possible.
- **Omitting UDL principles** — Focusing only on Tomlinson's three dimensions without addressing representation, action/expression, and engagement misses learners with disabilities and language differences whose needs require additional flexibility layers.
...
- When the learning objective requires all students to practice the exact same procedure to build automaticity (e.g., a mathematics fluency drill or a standardized test preparation exercise) — uniform practice is appropriate for building specific procedural fluency
- When the group is genuinely homogeneous in readiness and there is no meaningful variance in learning profiles — differentiation overhead is not justified when all students are within a narrow readiness band for a specific skill
