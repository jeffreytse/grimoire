...
1. **Draft the Objective** — Write one sentence describing a qualitative, inspiring outcome. It should answer "Where do we want to go?" and be achievable in one quarter. Use strong verbs: "Establish," "Become," "Deliver," "Win."
2. **Write 2–5 Key Results** — Each KR must be measurable, binary-passable (yes/no or a number), and directly prove the Objective was met. Format: "[Verb] [metric] from [baseline] to [target] by [date]."
3. **Stress-test each KR** — Ask: "If we hit this number but the Objective clearly failed, is the KR wrong?" If yes, revise. KRs must collectively prove the Objective.
4. **Set ambitious targets** — OKR targets should feel like 70% achievement is a strong result. If 100% feels easy, the target is a task list, not an OKR.
5. **Align vertically and horizontally** — Check that team OKRs ladder up to company OKRs. Check peer teams' OKRs for conflicts or missing dependencies.
6. **Assign owners** — Each KR has one accountable owner. Teams can contribute, but one person drives measurement and reporting.
7. **Establish check-in cadence** — Schedule weekly or bi-weekly confidence scores (0–10) and a written blocker. At quarter end, grade each KR 0.0–1.0.
