...
**Impact:** Painters who complete value studies before color work report 50–70% fewer major compositional revisions after the underpainting stage; Gurney documents that concept illustration approval rates double when value thumbnails are submitted before color comps
**Why best:** The human visual system is more sensitive to value contrast than to hue contrast — value structure determines readability, focal hierarchy, and spatial depth; color applied over a flawed value structure cannot fix the underlying failures
...
- Value structure is decided before color; never let color choices drive value relationships.
- The lightest light and darkest dark must not appear in equal measure throughout the composition; one or the other must dominate to establish the key.
- Shadow values must be cooler or warmer than light values, never neutral gray — colorless shadows destroy the illusion of a coherent light environment.
- Never correct a value failure with saturation; a color that is wrong in value will not be fixed by making it brighter.

## Common Mistakes

- **Too many value steps in the initial study** — planning with eight or ten steps is premature detail that obscures the primary mass relationships; simplify first, refine later.
- **Treating shadows as local color darkened** — mixing a darker version of an object's local color for shadows produces a flat, airless result; shadows are always influenced by reflected light and are relatively cool or warm independent of the object's local color.
- **Forgetting the value of sky and background** — backgrounds are often left too light by default, stealing contrast from the focal area; design background value intentionally.
- **Ignoring the value study once painting begins** — artists often abandon the study once inspired by color, losing the planned structure; pin the value study next to the canvas throughout the session.

## When NOT to Use

- When producing abstract or non-representational work where value hierarchy is deliberately destabilized
- When working in a fully prescribed illustrative style where the client has already approved a color comp (value is locked into the approved comp)
- When doing quick observational color sketches where capturing light quality takes priority over premeditated composition
