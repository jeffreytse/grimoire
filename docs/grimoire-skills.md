...
→ suggest-practice classifies this as engineering/debugging, routes to the appropriate skill
```

```
User: "I need to write a performance review for a direct report."
→ suggest-practice: no installed skill matches — suggests installing a management domain
```
...
→ plan-solution decomposes: legal entity structure → cap table clean-up →
...
→ review-practice-fit evaluates against applicable skills:
...
**What it produces:** A complete SKILL.md with all required sections: frontmatter, Why This Is Best Practice (Adopted by / Impact / Why best / Sources), and Steps. Structured to pass `review-skill`.

**Example invocation:**
```
User: "I want to add a skill for conducting blameless post-mortems."
→ write-skill guides: qualify the practice → name it → write frontmatter →
...
**Example invocation:**
```
Applying review-skill to skills/engineering/development/skills/review-pull-request/SKILL.md
...
**When to use:** When an existing SKILL.md has `review-skill` findings to address, a citation has become inaccurate, steps reference an outdated tool, or scope needs adjusting.

**What it produces:** Targeted changes to the specific failing sections only. Passing sections are left untouched.

**Example invocation:**
```
review-skill found: Impact line says "significantly improves" — no number or study
→ revise-skill: load the Impact finding → find the specific sentence →
  replace with causal argument or cited number → re-run review-skill → PASS
```
...
→ deprecate-skill: add notice → point to replacement skill →
...
**What it produces:** A complete domain scaffold: directory structure, domain plugin.json, sub-domain plugin.json files, marketplace.json entries, README table update, and minimum 2 seed skills — all verified by `audit-domain` before PR.

**Example invocation:**
```
User: "I want to add a 'leadership' domain with skills for 1:1s, performance reviews, team structure."
→ design-domain: qualify (3+ separable subdomains?) → choose flat vs hierarchical →
  create dirs → write plugin.json → write 2 seed skills → audit-domain → open PR
```
...
Describe situation
  → suggest-practice
       ...
       → multi-domain problem: hand off to plan-solution
...
suggest-practice            ← check no duplicate exists
  └→ design-domain          ← if new domain needed
       └→ write-skill       ← author SKILL.md
            └→ review-skill ← self-review
                 └→ revise-skill   ← fix NEEDS-REVISION findings
                      └→ review-skill   ← re-verify → PASS
                           └→ open PR
```
...
```
audit-domain                ← weekly or pre-release
  └→ NEEDS-REVISION found → revise-skill → re-run review-skill
  └→ REJECT found
       → outdated practice: deprecate-skill
       → fixable: revise-skill
       → replaced by new domain: design-domain
```
...
|-------|----------|---------|--------|
| `suggest-practice` | User | any situation or question | matching skill, ranked list, or install recommendation |
| `plan-solution` | User | multi-domain or complex problem | sequenced skill application plan |
| `review-practice-fit` | User | existing solution to evaluate | ALIGNED/PARTIAL/MISSING per practice + fix list |
| `write-skill` | Contributor | new practice to encode | complete SKILL.md |
| `review-skill` | Contributor | SKILL.md to evaluate | PASS/NEEDS-REVISION/REJECT verdict with findings |
| `revise-skill` | Contributor | review findings to address | targeted fixes to existing SKILL.md |
| `audit-domain` | Contributor | domain to assess | per-skill verdicts + summary counts |
| `deprecate-skill` | Contributor | outdated skill | deprecation notice + removal PR |
| `design-domain` | Contributor | new domain concept | directory scaffold + plugin files + seed skills |
