...
1. **Identify the adversarial condition.** Before applying this skill, confirm that the information source has motivation to mislead. The adversarial motivation may be: financial (seller hides liabilities, job candidate inflates credentials), political (ally conceals own failures), competitive (opponent feeds false intelligence), or relational (party avoids embarrassing truth). If no adversarial motivation exists, standard verification is sufficient.

2. **Map the key claims and their dependencies.** List the specific claims that, if false, would materially change your decision. For each claim, identify: what would have to be true in the world for this claim to be correct? What physical evidence, third-party records, or verifiable events would exist if this claim is true that would not exist if it is false?

3. **Design verification tests.** For each material claim, construct a test whose outcome depends on truth. Good tests have asymmetric outcomes: if the claim is true, the test produces result A; if false, result B. The party being verified should not know the test is happening or should not be able to manufacture the result. Examples:
   - Claim: "Revenue is $X." Test: request bank statements from the bank directly, not from the company. True revenue produces matching bank records; false revenue does not.
   - Claim: "Key customer X is committed long-term." Test: contact the customer independently without the counterparty's involvement. Committed customers confirm; uncommitted customers equivocate.
   - Claim: "The technology works at scale." Test: request access to production systems under load, not demo environments. Working technology performs; non-working technology fails under conditions the counterparty didn't anticipate.

4. **Map the information for internal inconsistencies.** Review all provided information for cases where two stated facts cannot both be true simultaneously. Inconsistencies do not prove deception — they may reflect error — but they identify claims requiring deeper verification. Common inconsistencies: financial figures that don't add to stated totals; timelines that contradict each other; capability claims inconsistent with known resource levels.

5. **Obtain independent corroboration from isolated sources.** For material claims, verify through sources who have not had the opportunity to coordinate their story with the primary source. Contact references before the counterparty knows you are contacting them; obtain documents from issuing institutions rather than from the party claiming them; interview multiple people with knowledge of the same events separately. When independent sources tell the same story without coordination, confidence rises; when independent sources tell different stories, investigate the discrepancy.

6. **Apply ACH: eliminate hypotheses by what they predict.** List the competing hypotheses (claim is true / claim is partially true / claim is false). For each hypothesis, identify what evidence it predicts. Collect the evidence. Eliminate hypotheses whose predictions are not borne out. The surviving hypothesis is the most defensible conclusion.

7. **Calibrate verification depth to stakes.** Full adversarial verification is costly. Apply depth proportionally: financial terms in a $10M acquisition warrant bank-level verification; a vendor's support hours claim warrants one reference call. For each material claim, estimate the cost of the claim being false and invest verification resources proportionally.

## Rules

- Absence of disconfirming evidence is not confirmation. If your verification found nothing wrong, check whether your tests were capable of detecting a problem. A verification test that only the truth would fail is evidence; a test that both truth and deception pass is not.
- Do not telegraph your verification tests to the counterparty. If they know what you're testing, they can manufacture confirming evidence. Design tests that are either invisible or whose results they cannot fabricate.
- Inconsistencies require follow-up, not immediate conclusions. An inconsistency between two stated facts indicates either deception, error, or incomplete information. Investigate which before concluding.
- Stop before full certainty — it is rarely achievable. The goal of adversarial verification is to reach a decision threshold, not absolute proof. When verification evidence is sufficient to make a decision at the confidence level required, stop. Additional verification returns diminish steeply.
...
- **Using counterparty-provided references and documents as primary verification:** The entire point of adversarial verification is to obtain evidence from sources the counterparty cannot control. References selected by the counterparty have selection bias; documents provided by the counterparty can be altered. Always seek independent sources.
- **Confirming rather than disconfirming:** The natural tendency is to seek evidence that confirms your current belief. In adversarial conditions, actively seek evidence that would disconfirm the claim. If you cannot find disconfirming evidence, your verification has more weight; if you were not looking for it, your verification means little.
- **Treating verification as a checkbox rather than an inference process:** Verification is not a list of boxes to check — it is an inference process. Evidence from verification should be evaluated in terms of how much it shifts your probability assessment that the claim is true.
- **Not calibrating depth to stakes:** Verifying low-stakes claims to high-depth wastes resources. Verify proportionally to the cost if the claim is false.
